...
\begin{proof}
Let $G$ be a digraph such that $C(G)$ has genus $g_{\rm{em}}(C(G)) \geq 1$ and such that each strongly connected component $G_i$ has genus $g_{\rm{em}}(G_i) \geq 1$, $i = 1, \ldots, n$. Since $g_{\rm{em}}(C(G)) \geq 1$, there is a directed cycle $c$ in $C(G)$ such that $[c] \not= 0$ in $H_{1}(S)$ for any embedding of (the subgraph) $c$ in any surface $S$. Suppose now that $p:H \to G$ is a directed emulator of $G$ such that $g(H) = g_{\rm{em}}(G)$. Consider any embedding of $H$ into a surface $\Sigma$ of genus $g(\Sigma) = g(H)$ witnessing the minimality of the genus. According to Prop.~\ref{prop:unreduced_condensation}, there is a commutative diagram
$$ \xymatrix{ H \ar[rr]^-{({\rm{pr}}_{H}, 1_{E_{H}})} \ar[d]_-{(p,q)} && C^{\circlearrowleft}(H) \ar[d]^{([p],q)} \\ 
G \ar[rr]^-{({\rm{pr}}_{G}, 1_{E_{G}})} && C^{\circlearrowleft}(G) 
}$$
Using the canonical inclusion graph morphism $C(G) \subseteq C^{\circlearrowleft}(G)$, we may regard the cycle $c$ as sitting inside $C^{\circlearrowleft}(G)$.  Let $\tilde{c}$ be a minimal (with respect to length) directed cycle in $C^{\circlearrowleft}(H)$ whose image in $C^{\circlearrowleft}(G)$ is $c$. Consider the two cases:
...
\noindent (2) $[\tilde{c}] \not= 0$ in $H_{1}(\Sigma)$. Since $ ({\rm{pr}}_{H}, 1_{E}): H \to C^{\circlearrowleft}(H)$ is onto, there exists $\beta \subset H$ that is sent onto $\tilde{c}$. Since $g(H) = g_{\rm{em}}(G) \geq \sum_{i} g_{\rm{em}}(G_i)$, $H$ supports a family $\{ \alpha_{k} \}_{k}$ of $2\sum_{i} g_{\rm{em}}(G_{i})$ of cycles that $\{ [\alpha_{k}] \}_{k}$ is a family of ${\mathbb{Z}}$-independent homology classes in $H_{1}(\Sigma)$. Suppose that a relation $[\beta] = \sum_{k} \lambda_{k} [\alpha_{k}]$ holds. This implies that there exists some (multi)cycle homologous to $\beta$ that is supported in 
