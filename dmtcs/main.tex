...
\begin{lemma} $C(G) = {\rm{Exc}}(C^{\circlearrowleft}(G))$.
\end{lemma}
...
Define $q': E_{C(G)} \to E_{C(H)}$ by restricting $q$ to $E_{G} - \{ e \in E_{G} \ | \ s(e) \sim_{V_{G}} t(e) \}$. 
Since $(p,q)$ is a graph morphism, $$[p](s_{C(G)}(e)) = [p]([s(e)]) = [p(s(e))] = [s(q(e))] = s_{C(H)}(q'(e))$$ and similarly $[q](t_{C(G)}(e)) = t_{C(H)}(q'(e))$, so $([p],q'): C(G) \to C(H)$ is a graph morphism. 
Suppose that we started from a directed emulator morphism. Since $(p,q)$ is onto, so is $([p],q')$. Let $[v] \in C(H)$ and let $e \in E_{C(H)}$. Choose a representative $v \in V_{H}$, that is a vertex inside the strongly connected component $C(v)$. By the lifting property for the directed emulator $(p,q): G \to H$, there is an edge $e' \in E_G$ such that $p(s(e')) = v$ and $q(e') = e$. But then $[p]([s(e')]) = [p(s(e')] = [v]$ and $q'(e') = q(e') = e$. Hence $([p],q')$ is a directed emulator morphism.
\end{proof}
