...
\end{prop}
\begin{proof}
First, define the natural map from a directed graph $G$ to its unreduced condensation graph as follows: send a vertex $v$ to its class $[v] = C(v)$ and let it be the identity on the edges. Then 
$$ {\rm{pr}}_{G} \circ s(e) = [s(e)] = s_{C^{\circlearrowleft}(G)}(e) = s_{C^{\circlearrowleft}(G)} \circ {1}_{E}(e)$$ and similarly ${\rm{pr}} \circ t(e) = t_{C^{\circlearrowleft}(G)} \circ 1_{E}(e)$. So $({\rm{pr}}_{G}, 1_{E_{G}})$ is a graph morphism. 

Next, a graph morphism $(p:V_{G} \to V_{H}, q:E_{G} \to E_{H})$ preserves strong connectedness, so there is an induced map $[p]: V_{C(G)} \to V_{C(H)}$ defined by $[p]([v]) = [p(v)]$ for any $[v] \in V_{G}/\!\!\sim_{V}$. 
...
\begin{remark} \label{rem:canonical_map_to_unreduced_condensation}
The canonical map $G \to C^{\circlearrowleft}(G)$ is itself a directed graph emulator morphism. 
\end{remark}
...
\noindent (2) $[\tilde{c}] \not= 0$ in $H_{1}(\Sigma)$. Since $ ({\rm{pr}}_{H}, 1_{E}): H \to C^{\circlearrowleft}(H)$ is onto, there exists $\beta \subset H$ that is sent onto $\tilde{c}$. Since $g(H) = g_{\rm{em}}(G) \geq \sum_{i} g_{\rm{em}}(G_i)$, $H$ supports a family $\{ \alpha_{k} \}_{k}$ of $2\sum_{i} g_{\rm{em}}(G_{i})$ of cycles that $\{ [\alpha_{k}] \}_{k}$ is a family of ${\mathbb{Z}}$-independent homology classes in $H_{1}(\Sigma)$. Suppose that a relation $[\beta] = \sum_{k} \lambda_{k} [\alpha_{k}]$ holds.
