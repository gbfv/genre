...
\begin{definition}
The {\emph{condensation graph}} Cond$(G)$ of $G$ is the directed graph determined by $$V_{\rm{Cond}(G)} = V/\!\! \sim_{V},  \ \ \ E_{\rm{Cond}(G)} = E - \{ e \in E \ | \ s(e) \sim_{V} t(e) \}$$
$$ s|_{\rm{Cond}(G)}(e) = [s(e)], \ \ \ t|_{\rm{Cond}(G)}(e) = [t(e)].$$
\end{definition} 
...
\begin{lemma}
Let $\sim_{E}$ be the equivalence relation defined on $E$ by $e \sim_{E} f$ if $s(e) \sim_{V} t(e)$ and let $C^{\circlearrowleft}(G) = (V',E')$ be the directed graph defined by $V' = V/\!\!\sim_{V}$, $E' = E/\!\!\sim_{E}$. Then $C(G) = {\rm{Exc}}(C^{\circlearrowleft}(G))$.
\end{lemma}

There is a partial order on the vertices of $C(G)$ defined by $u \leq v$ if there is a directed path from $u$ to $v$. Hence Cond$(G)$ is a directed acyclic graph.  It follows from definitions that two maximal elements cannot be adjacent and that removing a maximal element and its incoming edges yields another directed acyclic graph.
