...
\end{definition} 
\noindent Informally speaking, the {\emph{condensation}} graph ${\rm{Cond}}(G)$ is the result of contracting each strongly connected component to a vertex, removing every self-loop:

\begin{lemma}
Let $\sim_{E}$ be the equivalence relation defined on $E$ by $e \sim_{E} f$ if $s(e) \sim_{V} t(e)$ and let $G'$ be the directed graph defined by $V_{G'} = V/\!\!\sim_{V}$, $E_{G'} = E/\!\!\sim_{E}$. Then $\rm{Cond}(G) = {\rm{Exc}}(G')$.
\end{lemma}

There is a partial order on the vertices of ${\rm{Cond}}(G)$ defined by $u \leq v$ if there is a directed path from $u$ to $v$. Hence Cond$(G)$ is a directed acyclic graph.  It follows from definitions that two maximal elements cannot be adjacent and that removing a maximal element and its incoming edges yields another directed acyclic graph.
...
\begin{lemma}
A directed emulator morphism $G \to H$ induces a directed emulator morphism ${\rm{Cond}}(G) \to {\rm{Cond}}(H)$.
\end{lemma}

\begin{proof}
A graph morphism $(p:V_{G} \to V_{H}, q:E_{G} \to E_{H})$ preserves strong connectedness, so there is an induced map $[p]: V_{{\rm{Cond}}(G)} \to V_{{\rm{Cond}}(H)}$ defined by $[p]([v]) = [p(v)]$ for any $[v] \in V_{G}/\!\!\sim_{V}$. 
Define $q': E_{{\rm{Cond}}(G)} \to E_{{\rm{Cond}}(H)}$ by restricting $q$ to $E_{G} - \{ e \in E_{G} \ | \ s(e) \sim_{V_{G}} t(e) \}$. 
Since $(p,q)$ is a graph morphism, $$[p](s_{{\rm{Cond}}(G)}(e)) = [p]([s(e)]) = [p(s(e))] = [s(q(e))] = s_{{\rm{Cond}}(H)}(q'(e))$$ and similarly $[q](t_{{\rm{Cond}}(G)}(e)) = t_{{\rm{Cond}}(H)}(q'(e))$, so $([p],q'): {\rm{Cond}}(G) \to {\rm{Cond}}(H)$ is a graph morphism. 
Suppose that we started from a directed emulator morphism. Since $(p,q)$ is onto, so is $([p],q')$. Let $[v] \in {\rm{Cond}}(H)$ and let $e \in E_{{\rm{Cond}}(H)}$. Choose a representative $v \in V_{H}$, that is a vertex inside the strongly connected component $C(v)$. By the lifting property for the directed emulator $(p,q): G \to H$, there is an edge $e' \in E_G$ such that $p(s(e')) = v$ and $q(e') = e$. But then $[p]([s(e')]) = [p(s(e')] = [v]$ and $q'(e') = q(e') = e$. Hence $([p],q')$ is a directed emulator morphism.
\end{proof}
