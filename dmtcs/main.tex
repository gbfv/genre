...
We assume from the reader a working knowledge on directed graphs and automata theory (see for instance \cite{Sak03}). Here we review the notion of directed emulators and state the main results from we use in this paper. For more details, we refer to \cite[\S~4]{BD22}.

\begin{definition}
Let $G$ be a digraph. We say that a digraph $G'$ is a {\emph{directed emulator}} of $G$ if there is a directed graph morphism $(p, q): G' \to G$ such that $p$ is surjective and for any edge $e \in E_{G}$ and any $x \in p^{-1}(s_{G}(e))$, there is an edge $e' \in E_{G'}$ such that $s_{G'}(e') = x$ and $q(e') = e$. Such a pair $(p,q)$ is called a {\emph{directed emulator morphism}}. The set of directed emulators of $G$ is denoted ${\rm{em}}(G)$.
\end{definition}

Recall the definition of a graph $G$: it is the minimal genus $g(G)$ among all genera of (closed oriented) surfaces into which the graph $G$ embeds. 

\begin{definition}
Let $G$ be a directed graph. The {\emph{directed emulation genus}} of $G$ is $g_{\rm{em}}(G) = {\min}\ \{ g(H) \ | \ H \in {\rm{em}}(G) \}$.
\end{definition}
...
\begin{proof}
The identity map is a directed emulator morphism,
\end{proof}

We shall need the following observation.

\begin{lemma} \label{lem:cond_for_graph_iso}
If a directed emulator graph $H \to G$ restricts to an injection $V_{H} \to V_{G}$ on the set of vertices and $H$ is simple, then  it is a graph isomorphism.
\end{lemma}

\begin{proof}
Since a directed emulator graph is an epimorphism, the induced map $V_{H} \to V_{G}$ is bijective. Thus the lift of a directed edge is unique.
\end{proof}

Any automaton $\A$ gives rise to a directed graph ${\rm{G}}_{\A}$ by forgetting the extra structure (labels and distinguished initial and final states) on $\A$. We shall write $L(\A)$ for the language recognized by $\A$.

\begin{definition}
Let $L$ be a regular language. The {\emph{genus of the language}} $L$ is $$ g(L) = {\min}\ \{ g({\rm{G}}(\A)) \ | \ L(\A) = L, \ \A\ {\rm{deterministic}} \}.$$
\end{definition}

The following result is a direct consequence of the definition. 

\begin{lemma} \label{lem:basic_ineq_language_genus}
Let $\A_{\rm{min}}(L)$ be the minimal deterministic automaton recognizing $L$ and let $G(L) = {\rm{G}}_{\A_{\rm{min}}(L)}$. Then $g(L) \leq g(\A_{\rm{min}}(L))$.
\end{lemma}

\begin{remark}
Numerous examples of languages exist for which the inequality of Lemma~\ref{lem:basic_ineq_language_genus} is strict. 
\end{remark}
...
A regular language $L$ has genus less or equal to $g$ if and only if its minimal automaton has a directed emulator of genus less or equal to $g$. In particular, $g(L) = g_{\rm{em}}(G(L))$.
\end{theorem}



There is also a notion of emulator for undirected graphs. Reall that there is a forgetful functor  $U$ from the category of directed graphs to the category of undirected graphs. It follows from definitions that $g(G) = g(U(G))$.
...
Let $G = (V,E,s,t)$ be a directed graph. Two vertices $v, w \in V_{G}$ are {\emph{strongly connected}} if there exists a directed path from $v$ to $w$ and a directed path from $w$ to $v$. Strong connectedness is an equivalence relation $\sim_{V}$ on $V$. Let $C(v)$ be the set of all vertices that are strongly connected to a vertex $v$.
The subgraph $G_{\mid C(v)}$ is the {\emph{strongly connected component}} of $G$ containing $v$. If $G$ is finite, then it has a finite number of strongly connected components. 

\begin{definition}
The {\emph{unreduced condensation graph}} $C^{\circlearrowleft}(G)$ of $G$ is the directed graph defined by
$$ V_{C^{\circlearrowleft}(G)} = V/\!\! \sim_{V}, \ \ \ E_{C^{\circlearrowleft}(G)} = E, \ \ \ s_{C^{\circlearrowleft}(G)}(e) = [s(e)], \ t_{C^{\circlearrowleft}(G)}(e) = [t(e)].$$
The {\emph{condensation graph}} $C(G)$ of $G$ is the directed graph defined by $$V_{C(G)} = V_{C^{\circlearrowleft}(G)}  \ \ \ E_{C(G)} = E - \{ e \in E \ | \ s(e) \sim_{V} t(e) \},$$  
$$s_{C(G)}(e) = [s(e)], \ t_{C(G)}(e) = [t(e)].$$
\end{definition} 



\noindent Informally speaking, the {\emph{condensation}} graph $C(G)$ is the result of contracting each strongly connected component to a vertex, removing every self-loop. To give a precise meaning to this statement, we recall the {\emph{excision}} operation, Exc: it consists in removing all self-loops of a graph (see \cite[Def.~1]{BD22}).

\begin{lemma} $C(G) = {\rm{Exc}}(C^{\circlearrowleft}(G))$. In particular, $g_{\rm{em}}(C(G)) = g_{\rm{em}}(C^{\circlearrowleft}(G))$.
\end{lemma}

\begin{proof}
The first statement follows from the definition. The second statement follows from \cite[Th.~5]{BD22}. 
\end{proof}

There is a partial order on the vertices of $C(G)$ defined by $u \leq v$ if there is a directed path from $u$ to $v$. Hence 
$C(G)$ is a directed acyclic graph.  It follows from definitions that two maximal elements cannot be adjacent and that removing a maximal element and its incoming edges yields another directed acyclic graph.
...
\begin{lemma} \label{rem:canonical_map_to_unreduced_condensation}
The canonical map $G \to C^{\circlearrowleft}(G)$ is itself a directed graph epimorphism. It is not a directed emulator graph in general.
\end{lemma}

\begin{proof}
The natural map from a directed graph $G$ to its unreduced condensation graph is defined as follows: send a vertex $v$ to its class $[v] = C(v)$ and let it be the identity on the edges. Then 
$$ {\rm{pr}}_{G} \circ s(e) = [s(e)] = s_{C^{\circlearrowleft}(G)}(e) = s_{C^{\circlearrowleft}(G)} \circ {1}_{E}(e)$$ and similarly ${\rm{pr}} \circ t(e) = t_{C^{\circlearrowleft}(G)} \circ 1_{E}(e)$. So $({\rm{pr}}_{G}, 1_{E_{G}})$ is a graph morphism. It follows from the definition that it is an epimorphism.  To see why it is not a directed emulator graph in general, consider the following directed graph $G$ together with its canonical epimorphism onto $C^{\circlearrowleft}$ (dash dot denotes strongly components):
\begin{center}
\begin{tikzpicture}[->,transform shape,scale=0.5, baseline=-2mm]
\node[etat](G0) at (0,3.5){$v_1$};
\node[etat] (G1) at (0,2.5) {$v_2$};
\node[etat] (G2) at (3,3) {};
\node[etat] (F0) at (0,-3) {$[v_1]$};
\node[etat] (F1) at (3,-3) {};
\path (G0) edge [bend left] node [] {} (G1);
\path (G1) edge [bend left] node [] {} (G0);
\path (G0) edge [bend left] node [above] {\LARGE $e_1$} (G2);
\path (G1) edge [bend right] node [above] {\LARGE $e_2$} (G2);
\path (F0) edge [bend left] node [above]{\LARGE $e_1$} (F1);
\path (F0) edge [bend right] node [below]{\LARGE $e_{2}$} (F1);
\path (F0) edge [loop above] node [] {} (F0);
\path (F0) edge [loop below] node [] {} (F0);
\path[->] (1.5,1.5) edge [] node [] {} (1.5,-1.5);
\draw[red, dash dot] (0,3) circle[radius=1.1];
\draw[red, dash dot] (0,-3) circle[radius=1.2];
\draw[red, dash dot] (3,-3) circle[radius=0.8];
\draw[red, dash dot] (3,3) circle[radius=0.8];
\end{tikzpicture}
\end{center}
There is no lift of the edge $e_{2}$ starting at $v_1$.
\end{proof}



\begin{prop} \label{prop:unreduced_condensation}
A graph morphism $G \to H$ induces a commutative diagram of graph morphisms
$$
\xymatrix{ G \ar[rr]^-{({\rm{pr}}_{G}, 1_{E_{G}})} \ar[d]_-{(p,q)} && C^{\circlearrowleft}(G) \ar[d]^{([p],q)} \\ 
H \ar[rr]^-{({\rm{pr}}_{H}, 1_{E_{H}})} && C^{\circlearrowleft}(H) 
}
$$
Furthermore, if $G \to H$ is a directed emulator morphism then so is the induced map $C^{\circlearrowleft}(G) \to C^{\circlearrowleft}(H)$.
\end{prop}
\begin{proof}
A graph morphism $(p:V_{G} \to V_{H}, q:E_{G} \to E_{H})$ preserves strong connectedness, so there is an induced map $[p]: V_{C(G)} \to V_{C(H)}$ defined by $[p]([v]) = [p(v)]$ for any $[v] \in V_{G}/\!\!\sim_{V}$. 

Finally, this property ensures the commutativity of the diagram:
$$ ([p],q) \circ ({\rm{pr}}_{G}, 1_{E_{G}}) = ([p] \circ {\rm{pr}}_{G}, q) = ({\rm{pr}}_{H} \circ p, q) = ({\rm{pr}}_{H}, 1_{E_{H}}) \circ (p,q).$$

Let us prove the last statement of the proposition. Suppose that we started with a directed emulator morphism. Since $(p,q)$ is onto, so is $([p],q)$. Let $[v] \in C^{\circlearrowleft}(H)$ and let $e \in E_{C^{\circlearrowleft}(H)} = E_{H}$. Choose a representative $v \in V_{H}$, that is a vertex inside the strongly connected component $[v]$. By the lifting property for the directed emulator $(p,q): G \to H$, there is an edge $e' \in E_G$ such that $p(s(e')) = v$ and $q(e') = e$. But then $[p]([s(e')]) = [p(s(e')] = [v]$. Hence $([p],q)$ is a directed emulator morphism.
\end{proof}



\begin{prop} \label{prop:reduced_condensation}
A directed emulator morphism $G \to H$ induces a directed emulator morphism $C(G) \to C(H)$.
\end{prop}

\begin{remark}
In contrast to Prop.~\ref{prop:unreduced_condensation}, there is no natural map $G \to C(G)$. 
\end{remark}

\begin{proof}[Proof of Prop.~\ref{}]
By the argument above, there is an induced map $[p]: V_{C(G)} \to V_{C(H)}$ defined by $[p]([v]) = [p(v)]$ for any $[v] \in V_{G}/\!\!\sim_{V}$. 
Define $q': E_{C(G)} \to E_{H}$ by restricting $q$ to $E_{G} - \{ e \in E_{G} \ | \ s(e) \sim_{V_{G}} t(e) \}$. 
Since $(p,q)$ is a graph morphism, $q$ sends $\{ e \in E_{G} \ | \ s(e) \sim_{V_{G}} t(e) \}$ to $\{ e \in E_{H} \ | \ s(e) \sim_{V_{H}} t(e) \}$ so $q'$ is a map $E_{C(G)} \to E_{C(H)}$. Moreover, $$[p](s_{C(G)}(e)) = [p]([s(e)]) = [p(s(e))] = [s(q(e))] = s_{C(H)}(q'(e))$$ and similarly $[q](t_{C(G)}(e)) = t_{C(H)}(q'(e))$. Hence $([p],q'): C(G) \to C(H)$ is a graph morphism. 
Suppose that we started with a directed emulator morphism. Since $(p,q)$ is onto, so is $([p],q')$. Let $[v] \in C(H)$ and let $e \in E_{C(H)}$. Choose a representative $v \in V_{H}$, that is a vertex inside the strongly connected component $C(v)$. By the lifting property for the directed emulator $(p,q): G \to H$, there is an edge $e' \in E_G$ such that $p(s(e')) = v$ and $q(e') = e$. But then $[p]([s(e')]) = [p(s(e')] = [v]$ and $q'(e') = q(e') = e$. Hence $([p],q')$ is a directed emulator morphism.
\end{proof}
...
\begin{proof}[Proof of Theorem~\ref{th:planar_iff_scc_planar}]
...
We shall now prove that Theorem~\ref{th:planar_iff_scc_planar} does not generalize in genus $> 0$.  
...
Denote by $G(L)$ the underlying directed graph of the minimal deterministic automaton $\A$ recognizing $L$.

\begin{corollary} \label{cor:language_planar_iff_det_has_planar_scc}
A regular language $L$ is planar if and only if  every strongly connected component of $G(L)$ has a planar directed emulator.
\end{corollary}

This result was proved by Denis Kuperberg in \cite[Th.~21]{Kuperberg}.

\begin{proof}
Follows from Theorem~\ref{th:fund} and Theorem~\ref{th:planar_iff_scc_planar}.
\end{proof}

It is well known that the genus of a graph is additive on blocks \cite{Ad62}. In view of Theorem~\ref{th:planar_iff_scc_planar} and Corollary~\ref{cor:language_planar_iff_det_has_planar_scc}, it might be natural to believe that the genus of a language $L$ can be computed as the sum of the minimal genera of the directed emulators of the strongly connected components of $G(L)$. A key result is that a formula of this kind does not hold in higher genus.

\begin{theorem}
There exists a directed graph $G$ such that 
$g_{\rm{em}}(G) > \sum_{i} g_{\rm{em}}(G_i)$
where $G_1, \ldots, G_n$ are the strongly connected components of $G$.
\end{theorem}

\begin{proof}
We shall consider a digraph $G$ with the following properties:
\begin{enumerate}
\item[(1)] $G$ has $n > 1$ strongly connected components $G_{1}, \ldots, G_{n}$ such that $g_{\rm{em}}(G_{i}) = g_{i} \geq 1$ for $i = 1, \ldots, n$.
%\item[(2)] There is one and only one directed edge from one vertex of $G_{1}$ to one vertex of $G_{1}$ and no other edge between $G_{0}$ and the other strongly connected components $G_{2}, \ldots, G_{n}$.
\item[(2)] The condensation graph $C(G)$ has genus $g_{\rm{em}}(C(G)) \geq 1$.
\end{enumerate} 
%\begin{figure}
%\begin{center}
%\includegraphics[scale=0.5]{graph_with_components.pdf}
%\end{center}
%\caption{A schematic representation of the digraph $G$. Each box represents a directed subgraph. The subgraph $G_{2\ldots n}$ is the maximal directed subgraph of $G$ such that $V_{G_{2\ldots n}} = \{ G_2, \ldots, G_n \}$.}
%\end{figure} 
%Let $G_{1 \ldots n}$ be the directed subgraph of $G$ whose set of vertices is $\{G_1, \ldots, G_n\}$. It follows from the hypotheses that $g_{\rm{em}}(G_{1\ldots n}) = g_{\rm{em}}(C(G)) \geq 1$. Therefore there is a directed cycle $c$ -- which we may view as a subgraph -- in some new direction\footnote{See \cite[\S 4.3]{BD23} for the exact definition of the direction of an undirected graph.} for $U(C(G))$, supported by the vertices $G_1, \ldots, G_n$ only, such that $g_{\rm{em}}(c) = g_{\rm{em}}(C(G))$. This implies that the $1$-homology class
%$[c]$ in $H_{1}(S)$ is nonzero for any embedding of (the subgraph) $c$ in any surface $S$. 
Suppose that $p:H \to G$ is a directed emulator of $G$ such that $g(H) = g_{\rm{em}}(G)$.  According to Prop.~\ref{prop:reduced_condensation}, there is a directed emulator morphism $C(p): C(H) \to C(G)$. Consider the following two cases: 

\noindent (1) The induced map $V_{C(H)} \to V_{C(G)}$ is not bijective. Since it is surjective, $C(H)$ has strictly more vertices than $C(G)$. Thus $H$ has at least one extra strongly connected component $\tilde{G}_{i_{0}}$ lying over some strongly connected component $G_{i_{0}}$. Hence $$g(G) = g(H) \geq \sum_{j \not= i_{0}} g_{\rm{em}}(G_j) + 2g_{\rm{em}}(G_{i_{0}}) > \sum_{j} g_{\rm{em}}(G_{j}).$$\\

\noindent (2) The induced map $V_{C(H)} \to V_{C(G)}$ is bijective. Lemma~\ref{lem:cond_for_graph_iso} then implies that the original directed emulator morphism $C(H) \to C(G)$ is a graph isomorphism. In particular, the strongly connected components $\tilde{G}_{1}, \ldots, \tilde{G}_{n}$ of $H$ are in bijective correspondence with the strongly connected components $G_1, \ldots, G_n$ of $G$. Consider an embedding of $H$ into a surface $\Sigma$ of genus $g(\Sigma) = g(H) = g_{\rm{em}}(G)$ witnessing the minimality of the genus. By assumption, each strongly connected component $\tilde{G}_{i}$ lies in a subsurface $\Sigma_{i}$ of genus $g_{i} \geq 1$ inside $\Sigma$.
Furthermore, up to isotopy of $H$ in $\Sigma$, the subsurfaces can be chosen as to satisfy the following conditions:
\begin{enumerate}
\item[(1)] The full subgraph $\tilde{G_{i}}$ lies in the interior of $\Sigma_{i}$ for $i=1, \ldots, n$. Furthermore, a vertex $v \in V_{H}$ lies in $\Sigma_i$ if and only if $v \in G_{i}$, for $i = 1, \ldots, n$.
\item[(2)] $\Sigma_{i} \cap \Sigma_{j} = \varnothing$ if $i \not= j$.
\item[(3)] $\Sigma_{i}$ is the complement of a disc $D_{i}$ in a closed surface of genus $g_{i}$, for $i = 1, \ldots, n$. In particular, the boundary $\partial \Sigma_{i}$ of $\Sigma_i$ is a disc $C_{i} = \partial D_{i}$.
\end{enumerate}
The conditions imply that $\partial \Sigma_{i} \cap H$ consists of a finite number of points that belong to edges between vertices of $G_{i}$ and some other vertices in $G_{j}$ ($j \not= i$), for $i = 1, \ldots, n$.

On the one hand, each strongly connected component $\tilde{G}_{i}$ in $H$ has genus $g_{i}$ and therefore supports $2g_{i}$ directed cycles which induce independent $1$-homology classes in $H_{1}(\Sigma_{i})$. It follows that $H$ supports a family $\{ \alpha_{i} \}_{i}$ of $2\sum_{i} g_{\rm{em}}(G_{i})$ directed cycles that induce ${\mathbb{Z}}$-independent homology classes in $H_{1}(\Sigma)$. On the other hand, $C(G)$ has genus $g_{\rm{em}}(C(G)) \geq 1$, so there exists a directed cycle $c$ in $U(C(G)) = U(C(H))$ (regarded as a directed subgraph of ${U(C(H))}$ for some new choices of direction of edges) such that $g_{\rm{em}}(c) \geq 1$. It follows that the $1$-homology class $[c]$ is nonzero in $H_{1}(S)$ for any embedding of $c$ in any surface $S$. Since $ ({\rm{pr}}_{H}, 1_{E}): H \to C^{\circlearrowleft}(H)$ is onto, there exists a directed cycle $\beta \subset U(H)$ (for some new choices of direction of edges) that is sent onto $c$. Suppose that 
\begin{equation}
[\beta] = \sum_{j} a_{j}\ [\alpha_{j}]
\label{eq:linear_rel}
\end{equation}
is a nontrivial ${\mathbb{Z}}$-linear relation between $[\beta]$ and the family $\{ [\alpha_{i}] \}_{i}$. The quotient space obtained by collapsing each $\Sigma_{i}$ to a point $x_{i} \in \partial \Sigma_{i}$ is a closed surface $S$. The quotient map $q:\Sigma \to S$ induces a map $q_{*}:H_{1}(\Sigma) \to H_{1}(S)$ such that $q_{*}([\beta]) = [c] \not= 0$. Applying $q_{*}:H_{1}(\Sigma) \to H_{1}(S)$ to the linear relation $(\ref{eq:linear_rel})$  yields
$$ 0 \not= [c] = q_{*}([\beta]) = \sum_{j} a_{j} q_{*}([\alpha_{i}]) = 0, $$
which is absurd. (The equality $q_{*}([\alpha_{i}]) = 0$ follows
from the conditions (1)-(3) above.) Therefore the family $\{ [\beta], [\alpha_{i}] \}$ is still 
${\mathbb{Z}}$-independent in $H_{1}(\Sigma)$. But this shows that the genus of $\Sigma$ is greater than $\sum_{i} g_{i} = \sum_{i} g_{\rm{em}}(G_{i})$. This concludes the proof. 
%Since $g_{\rm{em}}(c) \geq 1$, $g_{\rm{em}}(\beta) \geq 1$ hence $[\beta]\not=0$ in $H_{1}(S)$ for any embedding of $\beta$ into any surface $S$.





%According to Prop.~\ref{prop:unreduced_condensation}, there is a commutative diagram
%$$ \xymatrix{ H \ar[rr]^-{({\rm{pr}}_{H}, 1_{E_{H}})} \ar[d]_-{(p,q)} && C^{\circlearrowleft}(H) \ar[d]^{([p],q)} \\ 
%G \ar[rr]^-{({\rm{pr}}_{G}, 1_{E_{G}})} && C^{\circlearrowleft}(G) 
%}$$


%Using the canonical inclusion graph morphism $C(G) \subseteq C^{\circlearrowleft}(G)$, we may regard the cycle $c$ as sitting inside $C^{\circlearrowleft}(G)$ (for some new direction of $C^{\circlearrowleft}(G)$).  Let $\tilde{c}$ be a minimal (with respect to length) directed cycle in $C^{\circlearrowleft}(H)$ whose image in $C^{\circlearrowleft}(G)$ is $c$. There are two cases to consider:
%
%\noindent (1) $[\tilde{c}] = 0$ in $H_{1}(\Sigma)$. Since by hypothesis $[c] \not= 0$ in $H_{1}(\Sigma)$, $\tilde{c}$ is not isomorphic to $c$ (as a directed graph). Therefore (lemma), the length of $\tilde{c}$ is strictly greater (a multiple) than the length of $c$. But then $\tilde{c}$ has more vertices than $c$, so $C^{\circlearrowleft}(H)$ has more vertices than $C^{\circlearrowleft}(G)$. Therefore $H$ has at least one extra strongly component $\tilde{G}_{i}$ that lies over $G_{i}$. Hence $g(G) \geq \sum_{j \not= i} g_{\rm{em}}(G_j) + 2g_{\rm{em}}(G_i) > \sum_{i} g_{\rm{em}}(G_{j})$.\\
%
%\noindent (2) Therefore, we are left to consider the case when any directed cycle $c$ as specified above has a preimage $\tilde{c}$ in $C^{\circlearrowleft}(H)$ such that $[\tilde{c}] \not= 0$ in $H_{1}(\Sigma)$. Taking for $c$ a directed cycle (possibly with repetitions) that runs over any edge of $U(C^{\circlearrowleft}(G))$ so that the underlying subgraph contains $U(C^{\circlearrowleft}(G))$, we deduce that 
%
%\noindent (2) $[\tilde{c}] \not= 0$ in $H_{1}(\Sigma)$. Since $ ({\rm{pr}}_{H}, 1_{E}): H \to C^{\circlearrowleft}(H)$ is onto, there exists a directed cycle $\beta \subset H$ that is sent onto $\tilde{c}$. Since $g(H) = g_{\rm{em}}(G) \geq \sum_{i} g_{\rm{em}}(G_i)$, $H$ supports a family $\{ \alpha_{i} \}_{i}$ of $2\sum_{i} g_{\rm{em}}(G_{i})$ directed cycles that are ${\mathbb{Z}}$-independent homology classes in $H_{1}(\Sigma)$. Consider the following diagram
%%$$\xymatrix{
%%\Sigma \ar[r] \Sigma/\cup_{
%%}$$
%
%Suppose that a relation $[\beta] = \sum_{k} \lambda_{k} [\alpha_{k}]$ holds. 
